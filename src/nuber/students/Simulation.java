...
public class Simulation {
	 ...
	 * @param maxSleep      The maximum amount a thread will sleep (in millseconds))
	 *                      to simulate driving to, or dropping off a passenger
	...
	public Simulation(HashMap<String, Integer> regions, int maxDrivers, int maxPassengers, int maxSleep,
		...
		// store a queue of all current bookings as Future's that will eventually give
		// us back a BookingResult object
		...
		// check that dispatch won't let us book passengers after we've told it to
		// shutdown
		if (dispatch.bookPassenger(new Passenger("Test", maxSleep),
				regionNames[new Random().nextInt(regionNames.length)]) != null) {
			throw new Exception(
					"Dispatch bookPassenger() should return null if passenger requests booking after dispatch has started the shutdown");
		}

		// whilst there are still active bookings, print out an update every 1s
		while (bookings.size() > 0) {

			// go through each booking, and if it's done, remove it from our active bookings
			// list
			...
			// print status update
			System.out.println(
					"Active bookings: " + bookings.size() + ", pending: " + dispatch.getBookingsAwaitingDriver());
